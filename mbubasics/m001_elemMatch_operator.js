...
db.movieDetails.find({ boxOffice: { $elemMatch:{ "country": "Germany", "revenue": {$gt: 15}}  } }).pretty()

db.movieDetails.find({boxOffice: {$elemMatch: {"country": "Germany", "revenue": {$gt: 16}}}})


//solution for challenge problme

db.scores.find({ "results": { $elemMatch: { { $gte: 70 }, { $lt: 80 } } } }).pretty() // incorrect syntax
db.scores.find({results: {$elemMatch: {$gte:70, $lt:80}}} ).pretty()  // correct syntax
